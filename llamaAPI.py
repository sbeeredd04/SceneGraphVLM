...
def setup_environment():
    """Setup Llama API configurations"""
    try:
        logger.info("Starting Llama API setup...")
        
        # Initialize the Llama API with your token
        llama = LlamaAPI("LA-c797a15457d24100b14dc7e0a8c5031a42add11f525f490da240294b316a2f5f")
        
        logger.info("Llama API setup completed")
        return llama
...
def predict_scene_graphs(train_graphs, num_predictions, model,
                        vocabulary_type='closed', object_classes=None, relationship_classes=None):
    """Generate scene graph predictions using Llama API"""
    try:
        logger.info(f"\n=== Starting Llama Scene Graph Prediction (Closed Vocabulary) ===")
        logger.info(f"Will generate {num_predictions} sequential predictions")
        ...
        for batch_start in range(0, num_predictions, n_frames_per_iteration):
            ...
            api_request_json = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a scene graph prediction model. Generate only valid JSON arrays containing scene graphs."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            if response and 'choices' in response:
                ...
                batch_predictions = parse_model_response(response_text)
                ...
                logger.warning(f"Failed to get valid response for batch starting at frame {batch_frame_nums[0]}")
                continue
...
def setup_output_directory(video_id):
    ...
    try:
        output_dir = f"./output_llama/{video_id}"
        ...
        for video_id in video_ids:
            try:
                ...
                if video_id != video_ids[-1]:  # Don't wait after the last video
                    logger.info("Waiting 120 seconds before next video...")
                    time.sleep(120)
